...
router.post('/cancel/', function(req, res){
  ...
  .then(function(leave){
    req.session.flash_message('La petición ha sido cancelada');
  })
  .catch(function(error){
    console.log('Ha ocurrido un error: '+error);
    req.session.flash_error('No se ha podido cancelar la petición de ausencia');
  })
  .finally(function(){
...
router.post(
  ...
  function(req, res){
    ...
    if (!validator.isNumeric(request_id)){
      req.session.flash_error('Error al rechazar la petición de ausencia');
    }

    if ( req.session.flash_has_errors() ) {
      console.error(
        'Han habido errores de validación al rechazar la peticion de ausencia para el usuario ' + req.user.id
      );
    ...
    .then(function(processed_leave){
      req.session.flash_message(
        'Tiene peticiones de ausencia para gestionar. '
        + (
          processed_leave.user.is_auto_approve()
          ? ''
          : 'Su supervisor tiene que aprobarlas todavía'
        )
      );
