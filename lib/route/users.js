...
router.post('/add/', function(req, res){
  ...
  .then(function(new_user_attributes){
      return model.User.find_by_email(new_user_attributes.email)
        .then(function(user){
          ...
          if (user) {
            req.session.flash_error('Correo electrónico ya está en uso');
  ...
  .then(function(){
      if ( req.session.flash_has_errors() ) {
          ...
          req.session.flash_message('Nueva cuenta creada correctamente');
    ...
    return model.User.find_by_email(new_user_attributes.email)
      .then(function(user){
        ...
        if (user && user.companyId !== employee.companyId) {
          var error_msg = 'Ya hay una cuenta activa con un correo parecido en el sistema.';
  ...
  if (
    ...
    return model.User
      ...
      .then(function(number_of_admins_to_be_left){
        ...
        req.session.flash_error('Este es el último administrador en el sistema. No se pueden quitar los permisos de adminsitrador.');
  ...
  .then(function(){
    ...
    employee.updateAttributes(new_user_attributes).then(function(){
      req.session.flash_message(
        'Información de '+employee.full_name()+' han sido actualizados'
      );
...
router.post('/delete/:user_id/', function(req, res){
    ...
    .then(function(result){
        req.session.flash_message(
            'Los datos del empleado han sido eliminados del sistema'
        );
        return res.redirect_with_session('../..');
    })
    .catch(function(error){
        ...
        req.session.flash_error(
            'Se ha producido un error al intentar eliminar el usuaro. ' + error
        );
...
router.all('/search/', function(req, res){
  ...
  if ( ! validator.isEmail( email )) {
    req.session.flash_error('El correo no pa rece ser válido: "'+email+'"');
    return res.json([]);
  }
