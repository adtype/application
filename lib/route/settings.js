...
router.post('/company/', function(req, res){
    ...
    .then(function(){
        req.session.flash_message('La configuración se ha actualizado correctamente');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        ...
        req.session.flash_error(
            'Error al intentar actualizar la info de empresa. Contacte con su administrador para reportar el problema.'
        );
  ...
  req.user.getCompany()
    ...
    .then(function(){
      req.session.flash_message( schedule.is_user_specific()
        ? 'El calendario ha sido guardado para el usuario'
        : 'El calendario ha sido guardado para la empresa'
      );
    })
    ...
    .catch(function(error){
      ...
      req.session.flash_error( schedule.is_user_specific()
        ? 'Error al intentar guardar el calendario del usuario'
        : 'Error al intentar guardar el calendario de la empresa'
      );
...
router.post('/departments/', function(req, res){
    ...
    .then(function(){
        if ( req.session.flash_has_errors() ) {
            ...
            req.session.flash_message('Cambios al departamento han sido guardados');
            return res.redirect_with_session('/settings/departments/');
        }
    })
    ...
    .catch(function(error){
        ...
        req.session.flash_error(
            'Error al actualizar los detalles del departments, contacte con el administrador del sistema'
        );
...
router.post('/departments/delete/:department_number/', function(req, res){
    ...
    if (!validator.isInt(department_number)) {
        ...
        req.session.flash_error('No se puede eliminar el departamento: parámetros inválidos');
    ...
    .then(function(company){
      ...
      if (department_to_remove.users.length > 0){
        req.session.flash_error(
          'No se puede eliminar el departamento '+department_to_remove.name
            +' debido a que aún tiene '
            +department_to_remove.users.length+' usuarios asociados a él.'
        );
...
router.post('/bankholidays/', function(req,res){
    ...
    .then(function(){
        ...
        if ( req.session.flash_has_errors() ) {
            ...
            req.session.flash_message('Los cambios han sido guardados');
            return res.redirect_with_session('/settings/general/');
        }
    })
    .catch(function(error){
        ...
        req.session.flash_error(
            'No se ha podido actualizar las fiestas nacionales, por favor contacte con el administrador'
        );
...
router.post('/bankholidays/delete/:bank_holiday_number/', function(req, res){
    ...
    if (!validator.isInt(bank_holiday_number)) {
        ...
        req.session.flash_error('No se puede eliminar la fiesta: parámetros incorrectos');
    ...
    .then(function(){
        req.session.flash_message('Se ha eliminado correctamente');
        return res.redirect_with_session('/settings/general/');
    });
});
...
router.post('/leavetypes', function(req, res){
    ...
    .then(function(){
        if ( ! req.session.flash_has_errors() ) {
            req.session.flash_message('Los cambios a los tipo de ausencias se han guardado correctamente');
        }
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        ...
        req.session.flash_error(
            'No se ha podido actualizar los detalles de los tipos de ausencias, contacte con el administrador.'
        );
...
router.post('/leavetypes/delete/:leave_type_number/', function(req, res){
    ...
    if (!validator.isInt(leave_type_number)) {
        ...
        req.session.flash_error('No se puede eliminar el tipo de ausencia: parámetros incorrectos');
    ...
    .then(function(company){
        ...
        // Check if there exist leaves for current type and if so, do not remove it
            ...
            req.session.flash_error('No se puede eliminar el tipo de ausencia: el tipo de ausencia está en uso');

            throw new Error('Failed to remove Leave type because it is in use.');
        }

        return leave_type_to_remove.destroy();
    })
    .then(function(){
        req.session.flash_message('El tipo de ausencia se ha eliminado correctamente');
        return res.redirect_with_session('/settings/general/');
    })
    .catch(function(error){
        console.error(
        ...
        req.session.flash_error(
          'No se puede eliminar el tipo de ausencia'
        );
