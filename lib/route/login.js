...
module.exports = function(passport) {
  ...
  router.post('/login', function(req, res, next) {
    passport.authenticate('local', function(err, user, info) {
      ...
      req.logIn(user, function(err) {
        ...
        req.session.flash_message('Bienvenido '+user.name+'!');
  ...
  router.post('/register', function(req, res){
      ...
      if (!email){
          req.session.flash_error('El correo no ha sido proporcionado');
      } else if ( ! email_validation_re.test( email )) {
          req.session.flash_error('El correo no es válido');
      }

      var name = req.param('name');
      if (!name){
          req.session.flash_error('El nombre no se ha especificado');
      }

      var lastname = req.param('lastname');
      if (!lastname) {
          req.session.flash_error('Los apellidos no han sido especificados');
      }
      ...
      if (!password) {
          req.session.flash_error('La contraseña no puede estar vacía');
      } else if ( password !== req.param('password_confirmed') ) {
          req.session.flash_error('La contraseña y la confirmación no coinciden');
      }

      var country_code = req.param('country');
      if (! validator.matches(country_code, /^[a-z]{2}/i) ){
          req.session.flash_error('Código de país incorrecto');
      }
      ...
      .catch(function(error){
          console.error(
              'Ha ocurrido un error al intentar crear la cuenta al usuario '
                  + email + ' : ' + error
          );

          req.session.flash_error(
            'No se ha podido registrar al usuario, por favor contacte al administrador del sistema.'+
  ...
  router.post('/forgot-password/', function(req, res){
    ...
    if (!email){
      ...
      req.session.flash_error('Correo electrónico no es válido');
    }
    ...
    var success_msg ='Verifique su buzón de entrada para más instrucciones';
    ...
    req.app.get('db_model').User.find_by_email(email)
      ...
      .catch(function(error){
        ...
        console.error('Ha ocurrido un error enviando el formulario para recuperar la contraseña: '+error);
        req.session.flash_error('No se ha podido proceder con la información enviada.');
        return res.redirect_with_session('./');
      });
  ...
  router.get('/reset-password/', function(req, res){
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      .then(function(user){
        if (! user) {
          req.session.flash_error('Link de reset inválido, por favor vuelva a solicitar el reset de la contraseña.');
  ...
  router.post('/reset-password/', function(req, res){
    ...
    if (password !== confirm_password) {
      req.session.flash_error('Contraseña confirmada no coincide con la contraseña original');
    ...
    req.app.get('db_model').User.get_user_by_reset_password_token(token)
      .then(function(user){
        if (! user) {
          req.session.flash_error('Link de reset desconocido, por favor envía la solicitud de nuevo');
          return res.redirect_with_session('/forgot-password/');
        }
      ...
      .then(function(){
        req.session.flash_message('Por favor use su nueva contraseña para acceder al sistema');
          return res.redirect_with_session('/login/')
      });
  });
    ...
    parseAsync.call(form, req)
      .then(function(args){
        ...
        if (files.company_dump.size === 0) {
          throw new Error('No se ha proporcionado un fichero para importar los datos');
        }
      ...
      .then(function(company){
        req.session.flash_message('Comañía '+ company.name +' ha sido restaurada');
        res.redirect_with_session('/import-company/');
      })
      .catch(function(error){
        req.session.flash_error('No se ha podido importar la empresa debido a errores: '+error);
        res.redirect_with_session('/import-company/');
      });
